...
Canvas paint_it_black(Canvas c) {
    int witdh;
    int height;
    for (witdh = 0; witdh < canvas_width(c); witdh++)
    {
        for (height = 0; height < canvas_height(c); height++)
        {
...
Canvas descending_diagonal(Canvas c) {
    ...
    int witdh;
    int witdh_bound = 0;
    // Äußere For-Schleife durchläuft alle Reihen von oben an absteigend (Durch 'height = height - 1').
    for (height = canvas_height(c) - 1; height >= 0; height = height - 1)
    {
        /*Innere For-Schleife färbt Pixel der Breite der Canvas mit 'witdh_bound' als Abbruchbedingung, welche Anfangs auf 0 gesetzt wird und
        nach jedem Durchlauf der Schleife in der äußeren For-Schleife innerhalb eines If-Statements um +1 erhöht wird (siehe Kommentar unten). 
        Der Laufparameter 'witdh' wird jede Iteration um +1 erhöht.*/
        for (witdh = 0; witdh <= witdh_bound; witdh++)
        {
            c = canvas_set_black(c, witdh, height);
        }
        /* If-Statement, um sicherzugehen, dass nach Bedingung der Aufgabe nach jeder Iteration der inneren For-Schleife ein weiteres Pixel der Breite nach eingefärbt wird 
        und gleichzeitig innere For-Schleife weiterläuft, wenn Canvas Breite erreicht wurde (else-Statement).*/
        if (witdh_bound < canvas_width(c) - 1)
        {
            witdh_bound = witdh_bound + 1;
        }
        else
        {
            witdh_bound = canvas_width(c) - 1;
        }
    }
    return c;
}
...
Canvas draw_rectangle(Canvas c, int x, int y, int width, int height) {
    //Question: Warum ist folgender Error enstanden, wenn ich einfach nur 'x' oder 'y' als Startwert in dei For-Schleifen übergeben habe:"./03ex.c:203:14: warning: expression result unused [-Wunused-value]"?
    // Antwort(?): Schleifenvariable muss Startwert haben, bzw. Synta muss folgendermaßen aussehen:"i = 0"?
    int x_value;
    int y_value;
    for (y_value = y; y_value > y - height; y_value = y_value - 1)
    {
        // printf("Outer For-Loop debug-test.");
        for (x_value = x; x_value < x + width; x_value++)
        {
            // printf("Inner For-Loop debug-test.");
            if (0 <= x_value && x_value <= canvas_width(c) - 1 && 0 <= y_value && y_value <= canvas_height(c) - 1)
            {
                // printf("If-Condition success!");
                c = canvas_set_black(c, x_value, y_value);
            }
        }
    }
    return c;
...
Canvas draw_rectangle_via_corners(Canvas c, int x0, int y0, int x1, int y1) {
    int witdh = x1 - x0;
    int height = y0 - y1;
    draw_rectangle(c, x0, y0, witdh + 1, height + 1);
    return c;
}